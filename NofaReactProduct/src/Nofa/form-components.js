...
export const FormInput = fieldRenderProps => {
  ...
  return <FieldWrapper>
        ...
        <div className={'k-form-field-wrap'}>
          <Input  valid={valid} type={type} id={id}  ariaDescribedBy={`${hintId} ${errorId}`} {...others} />
...
export const FormDateRangePicker = fieldRenderProps => {
  ...
  return <FieldWrapper style={wrapperStyle}>
        ...
        <DateRangePicker ariaLabelledBy={labelId} ariaDescribedBy={`${hintId} ${errorId}`}  valid={valid} id={id} disabled={disabled} {...others} />
        {showHint && <Hint id={hintId}>{hint}</Hint>}
        {showValidationMessage && <Error id={errorId}>{validationMessage}</Error>}
      </FieldWrapper>;
};
